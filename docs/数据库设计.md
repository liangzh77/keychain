...
5. 查询 user_permissions，只有存在且 allowed = 1 时，用户才允许调用该 provider/model。
6. 没有用户显式模型授权记录时默认拒绝。

以上判断决定用户是否可以调用某个 provider/model。渠道默认授权只用于后台管理和默认配置展示，不会在运行时分发中替代用户显式模型授权。key 级授权在 key 分发阶段继续执行，只影响具体分发哪一个 key。
