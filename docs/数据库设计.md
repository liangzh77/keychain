...
用户 key 权限用于限制某个用户在某个 provider 下可被分发的 key。未设置显式 key 权限时默认拒绝；运行时分发只会从 `allowed = 1` 的 key 中选择。
...
- Runtime API 输入 channel name；内部查询和关联使用 channel id。
...
其中 `user_key_permissions` 按 `user_id + provider_id + key_id` 左连接；没有显式记录时默认拒绝。
...
3. 更新 dispatch_logs.status 为 `FAILED`。
4. 如果错误明确表示 key 的额度、余额、点数已经耗尽，增加 api_keys.failure_count，更新 last_failed_at，并将 api_keys.is_available 设为 0。
5. 任务队列已满、任务被取消、任务不存在、文件大小超限、参数错误等请求级或临时错误只记录失败历史，不更新 key 可用性。
6. 提交事务。
